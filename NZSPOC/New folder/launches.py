...
DATA_PATH    = r'C:\Users\HP\scripts\NZSPOC\launches_next_past_month.csv'
...
def render_launch_card(
    ...
    wx_prob="Unknown", details_url=None, country=""
):
    wx_map = {
        "Low":   ("🟢", "Low"),
        "Medium":("🟠", "Medium"),
    ...
    with st.container():
        ...
        with cols[4]:
            if details_url and isinstance(details_url, str) and details_url.startswith("http"):
                st.link_button("Details", details_url)
            else:
...
with tab1:
    ...
    st_autorefresh(interval=5000, key="refresh")  # Animated refresh every 5 seconds
...
with tab3:
    ...
    if df.empty:
        ...
        # 3. Mission breakdown by Country
        ...
        if not mission_df.empty:
            mission_stats = mission_df['mission'].value_counts().reset_index()
        ...
        # 7. Success Rate by Country (Pie, selectable)
        ...
        # 8. Success Rate by Rocket (Pie, selectable)
        ...
        # 9. Timeline: Next 10 Scheduled Launches (Upcoming)
        ...
        # 10. First/Last launch date per country
        ...
        # 11. Failure Reason Breakdown (if present)
